...
object Channel {
  ...
  private val invalidChannelCharacters: List[Char] = ???
  ...
  final case class InvalidChannelLength(value: String) extends Throwable(s"Invalid channel length (${value.length}) $value")
  final case class InvalidChannelName(value: String) extends Throwable(s"Invalid channel name (contains $invalidChannelCharacters) $value")
  ...
  def apply(value: String) : ValidatedNel[Throwable, Channel] =
    (validChannelPrefix(value) *> validChannelLength(value) *> validChannelName(value)).map((v:String) => new Channel(v))
}
